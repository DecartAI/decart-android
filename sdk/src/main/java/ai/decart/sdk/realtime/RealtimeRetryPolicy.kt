...
/**
 * Reconnect / retry policy shared with the JS SDK ([config-realtime.ts]) and the
 * iOS [DecartRealtimeManager]. Lives separate from [RealtimeConfiguration] so it
 * stays internal and easy to override per-session if we ever need to.
 */
internal object RealtimeRetryPolicy {
    ...
    /**
     * Server / signaling error substrings that should NOT trigger a reconnect.
     * Matches the JS list verbatim and is checked case-insensitively against
     * the error message.
     */
    ...
    /** Exponential backoff in milliseconds, clipped to [MAX_DELAY_MS]. */
